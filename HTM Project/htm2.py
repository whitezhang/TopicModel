...
def main():
	...
	twitter_words_list = [voca.doc_to_ids(doc) for doc in (twitter_corpus)]
	...
	tcout = open(model + "tweets_pzd.txt", "a")
	for doc in twitter_words_list:
		print doc, type(doc)
		probs = []
		for z in range(options.K):
			prob = 1.0
			for w in doc:
				prob *= tweets_wt_distribution[z, w]
			probs.append(prob)
		# Normalization
		ssum = sum(probs)
		for n in probs:
			tcout.write(str(n/ssum) + " ")
		tcout.write("\n")
	tcout.close()
