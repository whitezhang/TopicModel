...
# ATM
def load_author(filename):
    lines = open(filename).readlines()
    author_list = []
    author_set = []
    for line in lines:
        info = line.split("\t")
        authors = [int(x) for x in info[0].split(" ")]
        author_list.append(authors)
        author_set.extend(authors)
    ...
    for authors in author_list:
        authors_ids = []
        for ai in authors:
            authors_ids.append(author_set.index(ai))
        ret_authors.append(authors_ids)
    return len(author_set), ret_authors
